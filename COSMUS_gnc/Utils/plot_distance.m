function [fig, M] = plot_distance(pos_history, ...
...
for k = 1:t_steps
    ...
    M{k} = compute_neighborhood(dist_k, r_comm, max_neig);
    dist_neig = dist_k(M{k} == true);
...
reference = yline(d,'--','LineWidth',2);
reference.Color = [0.25 0.25 0.25];

% Plot threshold
threshold = yline(2*r_coll,'-.','LineWidth',2);
threshold.Color = [0.25 0.25 0.25];

xlabel('Time [s]','fontsize',fontsize);
ylabel('Distance [m]','fontsize',fontsize);
h = legend('Average Dist.','Minimum Dist.','Maximum Dist.','Reference','Threshold','fontsize',fontsize);
