function Fragmentation_subplot(M, pos_hist, times, nb_agents, t_list)
    f = figure;
    ...
    for t_=1:6
        ...
        x_h = max(max(abs(X_c - min(x_axis)), abs(max(x_axis) - X_c)), 90) + e;
        y_h = max(max(abs(Y_c - min(y_axis)), abs(max(y_axis) - Y_c)), 90) + e;
        ...
        xlabel('x (pos)');
        ylabel('y (pos)');
        title(strcat('t= ',num2str(round(times(k))), ' [sec]'));
        drawnow;
        f.Position = [0 0 700 800];
        %clf(f);
    end
end